 ...
 */
 ...
 describe('Your test suite', () => {
    it('Ejercicio 1', () => {
        let result = largoDelNombre(45);
        assert.equal(result, 'El parámetro recibido es invalido');        
        result = largoDelNombre("Juan");
        assert.equal(result, 'Es un nombre corto. Su largo es de: 4');        
        result = largoDelNombre("Juan Lopez Gonzales");
        assert.equal(result, 'Es un nombre largo. Su largo es de: 19');
        result = largoDelNombre("JuanLope");
        assert.equal(result, 'Es un nombre OK. Su largo es de: 8');
    });

    it('Ejercicio 2', () => {
        let result = contarTipoNumber([45,4,"ddd",'ded',3]);
        assert.equal(result, 3);        
        result = contarTipoNumber(["ddd",'ded']);
        assert.equal(result, 0);        
        result = contarTipoNumber([3,5,3,2]);
    ...
    it('Ejercicio 3', () => {
      ...
      let result = datosPersona(persona);
      assert.equal(result, "Esta persona se llama Juan Carlos Lopez, tiene 55 años y su altura es de 1.87");              
    });
    
    it('Ejercicio 4', () => {
      let result = obtenerCantidadDePares(2,100);
      assert.equal(result, 'La cantidad de números pares obtenido fue de: 50');              
      result = obtenerCantidadDePares(3,100);
      assert.equal(result, 'La cantidad de números pares obtenido fue de: 49');              
      result = obtenerCantidadDePares(3,99);
      assert.equal(result, 'La cantidad de números pares obtenido fue de: 48');              

    });      

    
    it('Ejercicio 5', () => {
      let result = obtenerCantidadDeSedes(objetoFijo);
      assert.equal(result, 5);              
      result = obtenerCantidadDeSedes(numerosReales);
      assert.equal(result, "Error al obtener la cantidad de sedes: TypeError: Cannot read properties of undefined (reading 'sedes')");              
    });       

    it('Ejercicio 6', () => {
      let result = obtenerTelefonoPeru(objetoFijo);
      assert.equal(result, "5116358614");              
              
    });   
    
    
    it('Ejercicio 7', () => {
      let result = obtenerPaisesConTelefono(objetoFijo);
      assert.equal(result, 4);              
    ...
    it('Ejercicio 8', () => {
      let result = calculoMatematico("+",23,54);
      assert.equal(result, 77);              
      result = calculoMatematico("w",23,54);
      assert.equal(result, "La operación matemática que intenta realizar es inexistente");               
    });     

    it('Ejercicio 9', () => {
      let result = cargarArreglo(5);
      assert.equal(""+result,""+ [ 5,6,7,8,9,10]);              
      result = cargarArreglo(-5);
      assert.equal(result, "El número ingresado debe ser mayor a 0");               
    }); 
    
    it('Ejercicio 10', () => {
      let result =divisiblesPorCinco([51,60,75,883,91,10]);
      assert.equal(result,3);              
    }); 

    it('Ejercicio 11', () => {
      let numeros = [5,10,15,20,25,30];
      let result =sumatoria(numeros);
      assert.equal(result,105);              
    }); 

    it('Ejercicio 12', () => {
      let result =CantidadEmpleados(objetoFijo);
      assert.equal(result,false);              
    }); 

    it('Ejercicio 13', () => {
      let result =calcularCuadrados(arregloFijo);
      assert.equal("" + result,"" + [ 1,9,16,49,144,324,400,676]);              
    }); 

    it('Ejercicio 14', () => {
      let numeros = [5, 10, 15, 20, 25, 30, 35];
      let result =validarArreglo(numeros);
      assert.equal(result,true);              
      numeros = [1, 1, 1, 1, 1, 1, 1];
      result =validarArreglo(numeros);
      assert.equal(result,false);              
      numeros = [5, 10, 15];
      result =validarArreglo(numeros);
      assert.equal(result,false);              
    ...
    it('Ejercicio 15', () => {
      ...
      let result = validarNombre(nombres, "juan");
      assert.equal(result,true); 
       result = validarNombre(nombres, "Petronila");
      assert.equal(result,false);              
    }); 
    ...
    it('Ejercicio 16', () => {
      let numeros = [5, 10, 15, 20, 25, 30, 35];
      let result = devolverPosicion(numeros, 15);
      assert.equal(result,2); 
       result = devolverPosicion(numeros, 18);
      assert.equal(result,"No fue encontrado el número");              
    }); 


    it('Ejercicio 17', () => {
      let result = verificarSector(objetoFijo, "Contabilidad");
      assert.equal(result,true); 
      result = verificarSector(objetoFijo, "Mécanica");
      assert.equal(result,false); 
    }); 

    it('Ejercicio 18', () => {
      let result = verificarCantidadEmpleados(objetoFijo, 40);
      assert.equal(result,1); 
      
      result = verificarCantidadEmpleados(objetoFijo, 30);
      assert.equal(result,2);
      
      result = verificarCantidadEmpleados(objetoFijo, 20);
      assert.equal(result,4);  
      
      result = verificarCantidadEmpleados(objetoFijo, 10);
      assert.equal(result,5);  
    ...
    it('Ejercicio 19', () => {
      ...
      let result = verificarClave(alumno, "altura");
      assert.equal(result,false);
      
       result = verificarClave(alumno, "profesion");
      assert.equal(result,true); 
    ...
    it('Ejercicio 20', () => {
      ...
      const numerosR = [34.4552, 25.1543, 17.9438, 90.5, 67.82, 5, 9.99, 62.037];

      let result = quitarParteDecimal(numerosR);
      assert.equal("" + result, "" + [34,25,18,91,68,5,10,62]);
